...
use secp256k1::ffi::secp256k1_nonce_function_rfc6979;

use crate::hashes::TweakHash as _;

pub struct NonceFnData<'s, C: secp256k1::Signing> {
...
pub unsafe extern "C" fn s2c_ecdsa_nonce_fn<C: secp256k1::Signing>(
    nonce32: *mut c_uchar,
    msg32: *const c_uchar,
    key32: *const c_uchar,
    ...
    let data = data.cast::<NonceFnData<'_, C>>().as_mut().expect("required by SAFETY comment");

    // 2. The returned nonce is a secret nonce. Interpret it as a secret key
    //    and tweak it.
    let nonce32 = nonce32.cast::<[u8; 32]>().as_mut().expect("required by SAFETY comment");
    let Ok(sk) = secp256k1::SecretKey::from_byte_array(*nonce32)
    else {
        return 0
    };

    let untweaked_pubkey = sk.public_key(data.ctx);
    let tweak_bytes = crate::Sign2ContractHash::compute_tweak(&untweaked_pubkey, data.algo, data.data).to_byte_array();
    let tweak_sc = secp256k1::Scalar::from_be_bytes(tweak_bytes).expect("cryptographically unreachable");
    let tweaked_sk = sk.add_tweak(&tweak_sc).expect("cryptographically unreachable");
